...
export default function Page() {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between items-center text-center h-10 w-auto">
      ...
      <div className="flex items-center py-6">
        <form className="flex px-[4px] items-center text-center border rounded-3 border-gray-6 min-w-24 max-w-[313px] h-10">
          <Button className="p-0 hover:cursor-pointer flex items-center justify-center bg-transparent">
        ...
        <table className="min-w-full border-collapse border border-gray-100 ">
          <thead className="text-white ">
            <tr className="bg-blue-3">
              {showCheckboxes && (
                <th scope="col" className="px-1 sm:px-4 py-2 text-left">
                  <input
                    type="checkbox"
                    checked={selectAll}
              ...
              <th scope="col" className="px-1 sm:px-4 py-2 text-left">Название</th>
              <th scope="col" className="px-1 sm:px-4 py-2 text-left">Описание</th>
              <th scope="col" className="px-1 sm:px-4 py-2 text-left">Количество пользователей</th>
            </tr>
          </thead>
          <tbody>
            {rolesData.map((role, index) => (
              <tr className="border" key={index}>
                {showCheckboxes && (
                  <td className="px-1 sm:px-4 py-2 text-left">
                    <input
                      type="checkbox"
                ...
                <td className="px-1 sm:px-4 py-2 text-left">{role.name}</td>
                <td className="px-1 sm:px-4 py-2 text-left">{role.description}</td>
                <td className="px-1 sm:px-4 py-2 text-left">{role.userCount}</td>
              </tr>
            ))}
          </tbody>
        </table>
